...
export default {
  ...
  discoverTargets() {
    ...
    const allowlist = this.config.slices
    let names = readdirSync(slicesDir).sort()
    if (Array.isArray(allowlist)) names = names.filter(n => allowlist.includes(n))

    const slices = names.map(name => this.sliceTarget(name)).filter(Boolean)
