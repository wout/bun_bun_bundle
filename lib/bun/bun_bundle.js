...
export default {
  ...
  async buildAssets(type, options = {}) {
    const outDir = this.hanami ? this.outDir : join(this.outDir, type)
    ...
    for (const entry of entries) {
      ...
      const url = this.hanami
        ? `${this.config.publicPath}/${fileName}`
        : `${type}/${fileName}`
      this.manifest[key] = this.manifestEntry(url, content)
    }
  },
  ...
  async copyStaticAssets() {
    ...
    for (const dir of this.config.staticDirs) {
      ...
      for await (const file of glob.scan({cwd: fullDir, onlyFiles: true})) {
        ...
        const url = this.hanami
          ? `${this.config.publicPath}/${assetType}/${fileName}`
          : `${assetType}/${fileName}`
        this.manifest[`${assetType}/${file}`] = this.manifestEntry(url, bytes)
      }
    }
  },
  ...
  async writeManifest() {
    const manifestFullPath = this.hanami
      ? join(this.outDir, 'assets.json')
      : join(this.root, this.config.manifestPath)
